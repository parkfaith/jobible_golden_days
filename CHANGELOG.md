...
- **문제**: writings.json(글귀)이 bible.json(말씀)과 동일하게 성경 구절 직접 인용으로 채워져 있어 두 카테고리 간 차별성이 없었음
- **교체 내용**: 30개 항목 전부를 기독교 묵상 산문, 기도문, 신앙시로 대체
  - 토마스 아 켐피스 《그리스도를 본받아》
  - 오스왈드 챔버스 《주님은 나의 최고봉》
  - 아시시의 성 프란치스코 (평화의 기도)
  - 앤드루 머레이, 찰스 스펄전, 조지 뮬러, 존 번연, 줄리안 오브 노리치
  - 마이스터 에크하르트, 프랜시스 드 살레, 한나 위트홀 스미스, 존 웨슬리
  - 윤동주 (서시)
- **프롬프트 수정**: `WRITING_PROMPT`도 성경 구절 직접 인용 금지 → 묵상 산문/기도문/신앙시 생성으로 방향 변경 (이후 자동 생성도 동일 방향 유지)

**카테고리별 차별화 결과**:
| 카테고리 | 내용 |
|---------|------|
| 말씀 | 성경 구절 직접 인용 (권명 장:절) |
| 명언 | 기독교 사상가/신학자 격언 (짧은 한두 문장) |
| 시 | 찬송가 가사 (18~19세기 클래식 찬송가) |
| 글귀 | 기독교 묵상 산문/기도문/신앙시 (2~3문장) |

**수정 파일 목록 (Modified Files)**:

- `client/src/data/writings.json` (30개 항목 전면 교체)
- `scripts/lib/prompts.mjs` (WRITING_PROMPT 방향 변경)
- `CHANGELOG.md`
